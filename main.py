...
def wallet_sol_out(tx):
    """
    Πόσα SOL ΕΦΥΓΑΝ από το wallet σε αυτή τη συναλλαγή.

    ΤΟ BUG ΠΟΥ ΔΙΟΡΘΩΝΕΙ: παλιά μετρούσαμε μόνο τα nativeTransfers.
    Όμως τα περισσότερα DEX swaps γίνονται με WRAPPED SOL - τα SOL
    ταξιδεύουν ως tokenTransfer του mint So111...112, όχι ως native.
    Αποτέλεσμα: sol_spent ≈ 0, wallet_price γελοία χαμηλό, entry
    19x κάτω από την πραγματική τιμή, και μετά ψεύτικα +7447%.
    """
    native = sum(
        nt.get("amount", 0) for nt in (tx.get("nativeTransfers") or [])
        if nt.get("fromUserAccount") == WATCHED_WALLET
    ) / 1e9
    wsol = sum(
        t.get("tokenAmount", 0) or 0
        for t in (tx.get("tokenTransfers") or [])
        if t.get("mint") == WSOL_MINT
        and t.get("fromUserAccount") == WATCHED_WALLET
    )
    return native + wsol


def wallet_sol_in(tx):
    """Πόσα SOL ΜΠΗΚΑΝ στο wallet (native + wrapped)."""
    native = sum(
        nt.get("amount", 0) for nt in (tx.get("nativeTransfers") or [])
    ...
    wsol = sum(
        ...
        for t in (tx.get("tokenTransfers") or [])
        if t.get("mint") == WSOL_MINT
        and t.get("toUserAccount") == WATCHED_WALLET
    )
    return native + wsol
...
def process_transaction(tx, via="webhook"):
    try:
        ...
        sol_out = wallet_sol_out(tx)
        sol_in = wallet_sol_in(tx)

        if DEBUG_TX:
            print(
                f"TX {signature[:12]} [{source}/{tx_type}] "
                f"native={len(native_transfers)} "
                f"SOL out={sol_out:.6f} in={sol_in:.6f} | "
                + " ".join(
                    f"{(t.get('mint') or '')[:6]}:{t.get('tokenAmount')}"
                    f"{'→me' if t.get('toUserAccount') == WATCHED_WALLET else ''}"
                    f"{'me→' if t.get('fromUserAccount') == WATCHED_WALLET else ''}"
        ...
        for tt in token_transfers:
            ...
            if tt.get("toUserAccount") == WATCHED_WALLET:
                paper_buy(mint, amount, sol_out, signature, tx_time, via)
                ...
                paper_sell(mint, amount, sol_in, signature,
                           tx_time, ratio, via)
