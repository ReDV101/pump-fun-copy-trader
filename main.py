...
def get_jupiter_price_sol(mint, sol_amount, side="buy"):
    ...
    Δεν είναι εκτίμηση σαν το DexScreener - είναι το πραγματικό
    route με το πραγματικό price impact για ΑΥΤΟ το μέγεθος.
    ...
    try:
        ...
        if side == "buy":
            ...
            return None, None   # η πώληση υπολογίζεται ξεχωριστά

        res = requests.get(
            "https://quote-api.jup.ag/v6/quote", params=params, timeout=8
        )
        if res.status_code != 200:
            return None, None
    ...
    with state_lock:
        ...
        if state["balance_sol"] < total_cost:
            ...
            if MAX_PER_TOKEN_SOL > 0 and \
                    pos.get("invested_sol", 0.0) + PAPER_TRADE_SOL > MAX_PER_TOKEN_SOL:
