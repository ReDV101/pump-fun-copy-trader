...
def close_position(mint, exit_price, ratio=1.0, closed_by="wallet",
                   latency=None, exit_source=None, notify=True):
    """
    ...
    if notify:
        ...
        labels = {
            ...
            "manual": "✋ χειροκίνητο",
            "take_profit": f"🎯 auto take-profit (+{TAKE_PROFIT_PCT}%)",
        }
...
# ------------------------------------------------------------------
# 8b. AUTO TAKE-PROFIT
# Ελέγχει περιοδικά τις ανοιχτές θέσεις. Αν κάποια είναι +TAKE_PROFIT_PCT%
# πάνω από το μέσο entry (unrealized), κλείνει αυτόματα - ΑΝΕΞΑΡΤΗΤΑ
# από το τι κάνει ο trader. Αυτό είναι δική μας απόφαση ρίσκου, όχι
# αντιγραφή, οπότε καταγράφεται ξεχωριστά ("take_profit").
# ------------------------------------------------------------------
def take_profit_loop():
    if TAKE_PROFIT_PCT <= 0:
        print("Auto take-profit ανενεργό (TAKE_PROFIT_PCT <= 0).")
        return

    print(f"Auto take-profit ενεργό: +{TAKE_PROFIT_PCT}% "
        ...
        try:
            ...
            for mint, pos in positions.items():
                ...
                if gain_pct >= TAKE_PROFIT_PCT:
                    print(f"Take-profit trigger: {mint} στο "
                          f"{gain_pct:.1f}% (όριο {TAKE_PROFIT_PCT}%)")
                    close_position(
                        mint, exit_price, 1.0, "take_profit",
...
def build_portfolio_context():
    ...
    lines = [
        ...
        f"Auto take-profit: +{TAKE_PROFIT_PCT}%" if TAKE_PROFIT_PCT > 0 else "Auto take-profit: ανενεργό",
        "",
...
def handle_command(text):
    ...
    if cmd == "/start":
        ...
        send_telegram_message(
            ...
            f"Auto take-profit: "
            f"{f'+{TAKE_PROFIT_PCT}%' if TAKE_PROFIT_PCT > 0 else 'ανενεργό'}\n\n"
        ...
        if by_tp:
            msg += f"\n🎯 <b>Auto take-profit (+{TAKE_PROFIT_PCT}%)</b>\n"
