...
# Μυστικό για να μη μπορεί ο καθένας να στείλει fake webhook.
# Βάλε ό,τι θες (τυχαία γράμματα) και το ίδιο στο Helius dashboard.
...
# Το polling τώρα είναι μόνο δίχτυ ασφαλείας - τα webhooks κάνουν τη δουλειά
...
# Για να μην επεξεργαστούμε το ίδιο tx δύο φορές (webhook + polling)
seen_signatures = set()
seen_lock = Lock()


def already_processed(signature):
    """True αν το έχουμε ήδη δει. Αλλιώς το καταγράφει και επιστρέφει False."""
    with seen_lock:
        if signature in seen_signatures:
            return True
        seen_signatures.add(signature)
        # κράτα τη λίστα μικρή
...
def load_state():
    ...
    return {
        ...
        "last_signature": None,
        "total_fees_sol": 0.0,
        "latencies": [],          # ΝΕΟ: όλες οι μετρήσεις καθυστέρησης
    }
...
def paper_buy(mint, wallet_token_amount, wallet_sol_spent, signature,
    ...
    # --- ΜΕΤΡΗΣΗ ΚΑΘΥΣΤΕΡΗΣΗΣ ---
    # tx_time = πότε έγινε η συναλλαγή on-chain (unix seconds από Helius)
    # time.time() = πότε την είδαμε εμείς
    latency = None
    if tx_time:
...
# ------------------------------------------------------------------
# 7. ΕΠΕΞΕΡΓΑΣΙΑ TRANSACTION (κοινή για webhook & polling)
# ------------------------------------------------------------------
...
def process_transaction(tx, via="webhook"):
    try:
        ...
        tx_time = tx.get("timestamp")  # unix seconds, από το Helius
...
# ------------------------------------------------------------------
# 8. FLASK ROUTES - εδώ φτάνουν τα webhooks του Helius
# ------------------------------------------------------------------
@app.route("/")
def home():
...
def helius_webhook():
    ...
    try:
        ...
        # Το Helius στέλνει array από transactions
        txs = payload if isinstance(payload, list) else [payload]
        print(f"Webhook received: {len(txs)} tx(s)")

        # Απάντησε γρήγορα στο Helius, επεξεργάσου σε background
        Thread(
...
# ------------------------------------------------------------------
# 9. POLLING - δίχτυ ασφαλείας, σε περίπτωση που χαθεί webhook
# ------------------------------------------------------------------
        ...
        try:
            ...
            if txs:
                with state_lock:
                    ...
                    save_state()
                for t in txs:
                    already_processed(t["signature"])  # baseline, μην τα ξαναπιάσεις
...
def handle_command(text):
    ...
    if cmd == "/start":
        ...
        if not ls:
            send_telegram_message(
                "Καμία μέτρηση ακόμα. Θα εμφανιστούν μόλις γίνει το πρώτο trade."
            )
            return
        send_telegram_message(
        ...
        # ΣΗΜΑΝΤΙΚΟ: υπολόγισε και τις ανοιχτές θέσεις
        unrealized = 0.0
        for mint, pos in positions.items():
            cur = get_current_price_sol(mint)
            if cur:
