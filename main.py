...
def paper_buy(mint, wallet_token_amount, wallet_sol_spent, signature,
    ...
    # ------------------------------------------------------------------
    # ΔΙΟΡΘΩΣΗ: παλιά κάναμε skip όταν η τιμή έμοιαζε αφύσικη.
    # Αυτό έκοβε συστηματικά τα ΦΡΕΣΚΑ tokens (το DexScreener δεν έχει
    # προλάβει να τα κάνει index) - δηλαδή ακριβώς αυτά που κάνουν τα
    # μεγάλα κέρδη. Τώρα πέφτουμε πίσω στην τιμή του wallet.
    # ------------------------------------------------------------------
    price_source = "live"
    if live_price is None:
        live_price = wallet_price
        price_source = "wallet (χωρίς DexScreener)"
    elif wallet_price > 0 and (live_price / wallet_price > 50 or
                               live_price / wallet_price < 0.02):
        live_price = wallet_price
        price_source = "wallet (DexScreener αναξιόπιστο)"
