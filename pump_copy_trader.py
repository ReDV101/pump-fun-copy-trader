...
# ------------------------------------------------------------------
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN") or "8980136547:AAEI57YqbqhJWXESr_KNIUprMZ8cYBvZBc"
...
def short_mint(mint):
    return f"{mint[:4]}...{mint[-4:]}" if mint and len(mint) > 8 else (mint or "N/A")

# ------------------------------------------------------------------
# 6. DEXSCREENER PRICE FETCH
# ------------------------------------------------------------------
...
def paper_buy(mint, wallet_token_amount, wallet_sol_spent, signature):
    ...
    with state_lock:
        if state["balance_sol"] < PAPER_TRADE_SOL:
            if TELEGRAM_CHAT_ID:
                send_telegram_message(TELEGRAM_CHAT_ID, f"⚠️ Ανεπαρκές virtual balance για copy-buy σε {short_mint(mint)}")
            return
...
def handle_command(chat_id, text):
    ...
    if cmd == "/start":
        ...
        if not state["positions"]:
            send_telegram_message(chat_id, "Καμία ανοιχτή θέση αυτή τη στιγμή.")
            return
        ...
        with state_lock:
            ...
            state["closed_trades"] = []
            save_state()
        send_telegram_message(chat_id, f"🔄 Το paper account έγινε reset. Balance: {STARTING_BALANCE_SOL:.3f} SOL")
        ...
        if ai_client:
            try:
                ...
                send_telegram_message(chat_id, response_ai.text)
            except Exception as ai_err:
                send_telegram_message(chat_id, f"⚠️ Σφάλμα AI: {ai_err}")
        else:
            send_telegram_message(chat_id, "⚠️ Το GEMINI_API_KEY δεν έχει ρυθμιστεί.")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for update in data.get("result", []):
                    ...
                    if text:
                        print(f"RECEIVED MSG: '{text}' FROM CHAT_ID: {sender_id}")
                        # Αν το TELEGRAM_CHAT_ID δεν έχει οριστεί στο Railway, το ορίζουμε αυτόματα από το 1ο μήνυμα!
                        if not TELEGRAM_CHAT_ID:
                            TELEGRAM_CHAT_ID = sender_id
                            print(f"AUTO-SET TELEGRAM_CHAT_ID TO: {TELEGRAM_CHAT_ID}")
                        
                        if sender_id == str(TELEGRAM_CHAT_ID):
                            handle_command(sender_id, text)
                        else:
                            print(f"Ignored message from unauthorized chat: {sender_id}")
            else:
